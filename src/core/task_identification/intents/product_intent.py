...
import json
from src.services.store.woocommerce.data import PRODUCT_TYPES
...
async def identify_product_intent(
    ...
    """
    זיהוי כוונות הקשורות למוצרים בהודעת המשתמש
    ...
    if is_product_creation_intent(message):
        # חילוץ נתוני מוצר מההודעה
        product_data = extract_product_data(message)
        
        # חילוץ מזהה מוצר אם קיים (למקרה של עדכון)
        ...
        # יצירת שאלות להשלמת מידע חסר
        questions = generate_missing_field_questions(product_data) if missing_fields else []
    ...
    # בדיקה אם יש בקשה לקבלת מידע על מוצר ספציפי
    product_id = extract_product_id(message)
    if product_id:
        return IntentRecognitionResult(
            intent_type="get_product",
            confidence=0.8,
            params={"product_id": product_id},
            source="product_intent"
        )
    
    # בדיקה אם יש ביטויים הקשורים לרשימת מוצרים
    product_list_keywords = [
        "רשימת מוצרים", "כל המוצרים", "הצג מוצרים", "הראה מוצרים", 
        "product list", "all products", "show products", "list products"
    ]
    
    if any(keyword.lower() in message.lower() for keyword in product_list_keywords):
        return IntentRecognitionResult(
            intent_type="list_products",
            confidence=0.75,
            params={},
            source="product_intent"
        )
    
    # אם לא זוהתה כוונה ספציפית למוצרים
    return None
