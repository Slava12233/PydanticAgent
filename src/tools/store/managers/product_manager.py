...
from src.tools.store.managers.product_formatter import (
    ...
    async def _find_or_create_category(self, category_name: str) -> Optional[int]:
        ...
        try:
            ...
            if success and "id" in response:
                ...
                logger.error(f"שגיאה ביצירת קטגוריה: {message}")
                return None
                
        except Exception as e:
            logger.error(f"שגיאה ביצירת קטגוריה: {str(e)}")
            return None
