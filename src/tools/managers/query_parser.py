...
def parse_complex_query(text: str) -> List[Dict[str, Any]]:
    """
    פירוק שאילתה מורכבת למספר משימות פשוטות
    ...
    # בדיקה אם זו שאלה השוואתית
    if is_comparative_query(text):
        return parse_comparative_query(text)
    
    # בדיקה אם זו שאלה היפותטית
    if is_hypothetical_query(text):
        return parse_hypothetical_query(text)
