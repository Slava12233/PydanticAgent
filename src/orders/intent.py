...
async def identify_order_intent(
    message: str,
    context: Optional["TaskContext"] = None
) -> Optional["IntentRecognitionResult"]:
    """
    זיהוי כוונות הקשורות להזמנות בהודעת המשתמש
    ...
    # בדיקה אם יש כוונת ניהול הזמנות
    is_intent, intent_type = is_order_management_intent(message)
    
    if not is_intent:
        return None
    
    # טיפול בכוונות שונות
    if intent_type == "get_order":
        # חילוץ מזהה הזמנה
        order_id = extract_order_id(message)
        missing_info = [] if order_id else ["order_id"]
        ...
        return IntentRecognitionResult(
            ...
            params={
                ...
                "questions": generate_order_management_questions(intent_type, missing_info)
            },
            source="order_intent"
        )
    
    elif intent_type == "get_orders":
        # חילוץ פילטרים להזמנות
        filters = extract_order_filters(message)
        ...
        # חילוץ מזהה הזמנה וסטטוס
        order_id = extract_order_id(message)
        status = extract_order_status(message)
