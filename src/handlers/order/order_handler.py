...
class OrderHandler(BaseHandler):
    ...
    async def _handle_refund(self, params: Dict[str, Any]) -> HandlerResponse:
        """מטפל בביצוע החזר כספי."""
        try:
            # כאן יבוא הקוד לביצוע החזר כספי
            return HandlerResponse(
                success=True,
            ...
            return HandlerResponse(
                success=False,
                message=f"שגיאה בביצוע ההחזר הכספי: {str(e)}",
                error="REFUND_ERROR"
            )
    ...
    async def _handle_shipping(self, params: Dict[str, Any]) -> HandlerResponse:
        """מטפל בעדכון ומעקב אחר משלוח."""
        try:
            # כאן יבוא הקוד לטיפול במשלוח
            return HandlerResponse(
                success=True,
                message="פרטי המשלוח עודכנו בהצלחה",
                data={
                    "order_id": params.get("id"),
                    "tracking_number": "123456789"  # לדוגמה
                }
            )
        except Exception as e:
            return HandlerResponse(
                success=False,
                message=f"שגיאה בטיפול במשלוח: {str(e)}",
