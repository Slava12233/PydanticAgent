...
from ..utils.telegram_bot_utils import (
...
class TelegramBotHandlers:
    ...
    async def handle_message(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        # בדיקה אם המשתמש מבקש ליצור מוצר חדש
        create_product_keywords = [
            "ליצור מוצר", "יצירת מוצר", "להוסיף מוצר", "הוספת מוצר", "מוצר חדש",
            "רוצה ליצור מוצר", "אני רוצה ליצור מוצר", "אפשר ליצור מוצר"
        ]
        
        # בדיקה אם ההודעה מכילה מילות מפתח ליצירת מוצר
        if any(keyword in message_text for keyword in create_product_keywords):
            await update.message.reply_text(
                "כדי ליצור מוצר חדש לחנות שלך, אנא השתמש בפקודה /create_product"
            )
            return
