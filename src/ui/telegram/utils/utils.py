        ...
        if "can't parse entities" in str(e) and parse_mode:
            try:
                ...
                if parse_mode == ParseMode.MARKDOWN:
                    ...
                    # ניקוי תגיות MARKDOWN_V2 - כולל תווים מיוחדים נוספים
...
def format_error_message(text: str) -> str:
    ...
    return f"❌ {escape_markdown_v2('*')}שגיאה{escape_markdown_v2('*')}: {escaped_text}"
...
def format_success_message(message: str) -> str:
    ...
    return f"✅ {escape_markdown_v2('*')}הצלחה{escape_markdown_v2('*')}: {escaped_message}"
...
def format_warning_message(message: str) -> str:
    ...
    """
    escaped_message = escape_markdown_v2(message)
    return f"⚠️ {escape_markdown_v2('*')}אזהרה{escape_markdown_v2('*')}: {escaped_message}"
...
def escape_markdown_v2(text: str) -> str:
    """
    מנקה תווים מיוחדים של Markdown V2
    
    Args:
        text: הטקסט לניקוי
        
    Returns:
        טקסט נקי מתווים מיוחדים של Markdown V2
    """
    if not text:
        return ""
    
    # תווים שיש לברוח מהם ב-Markdown V2
    special_chars = ['_', '*', '[', ']', '(', ')', '~', '`', '>', '#', '+', '-', '=', '|', '{', '}', '.', '!', '\\']
    
    # הוספת תו בריחה לכל תו מיוחד
    result = ""
    for char in text:
